...
import { ArticleContent, ArticleInfo, Level2Category } from "./types";
...
const fetchArticleContent = async (href: string): Promise<ArticleContent> => {
...
export const useArticleContent = (href: string) =>
  useQuery<ArticleContent>({
    ...
    staleTime: 5 * 60 * 1000, // 선택적으로 추가
    enabled: !!href && href.length > 0, // href가 있을 때만 쿼리 실행
  });
