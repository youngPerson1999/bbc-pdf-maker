...
import { ArticleDto, ArticleInfoDto } from '@/api/generated/schemas';
...
const fetchArticleContent = async (href: string): Promise<ArticleDto> => {
  const res = await fetch(`/api/getArticle?href=${href}`);
...
export const useArticleContent = (href: string) =>
  useQuery<ArticleDto>({
    queryKey: ['articleContent', href],
    queryFn: () => fetchArticleContent(href),
    staleTime: 5 * 60 * 1000, // 선택적으로 추가
    enabled: !!href && href.length > 0,
  });
